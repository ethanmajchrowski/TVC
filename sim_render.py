...
def render_sim(simulation: Simulation):
    ...
    rocket_bottom = rocket.pos + pg.Vector2(0, rocket.length // 2).rotate(rocket.rot)
    # renderer.draw_circle((30, 30), 25, (255, 255, 255), camera_offset=True)
    # draw rocket body
    renderer.draw_polygon(get_rotated_rect(pg.Rect(rocket.pos, (10, rocket.length)), rocket.rot, rocket.pos), (80, 120, 80), camera_offset = True)
    ...
    renderer.draw_line(rocket_bottom, rocket_bottom + pg.Vector2(0, 50).rotate(rocket.rot + rocket.engine_angle), (255, 0, 255), 3, camera_offset=True)
    renderer.draw_line(rocket_bottom, (rocket_bottom.x, rocket_bottom.y + pg.Vector2(0, 50).rotate(rocket.rot + rocket.engine_angle).y), (255, 0, 0), 3, camera_offset=True)
    renderer.draw_line(rocket_bottom, (rocket_bottom.x + pg.Vector2(0, 50).rotate(rocket.rot + rocket.engine_angle).x, rocket_bottom.y), (0, 0, 255), 3, camera_offset=True)
